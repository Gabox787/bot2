...
@app.route('/')
def home(): return f"Grid Bot is running since {start_time_str}"
...
async def monitor_market(bot):
    ...
    stats["target_buy"], stats["target_sell"] = buys[0], sells[0]
    ...
    while True:
        try:
            ...
            if stats["current_price"] <= buys[0] and stats["last_buy_price"] is None:
                ...
                await bot.send_message(chat_id=config.CHAT_ID, text=f"📉 **BUY** [{now}] по `{stats['last_buy_price']}`")
